...
> **오늘의 목표**: 패키지 관리자로 프로그램을 깔끔하게 설치·삭제하고, fnm으로 Node.js 버전을 관리하고, WSL로 리눅스 환경을 만들어 봅니다.
...
설치하고, 확인하고, 삭제하는 세 단계가 전부입니다. 깔끔하게 사라지는 걸 확인해 보세요.
...
### STEP 4: fnm 셸 설정 (매우 중요!)

설치 후 터미널이 fnm을 자동으로 인식하도록 설정을 추가해야 합니다. 이 단계를 빠뜨리면 `fnm: command not found` 에러가 납니다.
...
### STEP 6: WSL 설치하기 (Windows 사용자만)

> 맥 사용자는 이미 유닉스 기반이므로 WSL이 필요 없습니다. 이 STEP은 건너뛰세요.
...
| `fnm: command not found` | 셸 설정 누락 또는 터미널 재시작 안 함 | STEP 4 설정 추가 후 터미널 껐다 켜기 |
...
| WSL 설치 명령에 반응 없음 | Windows 버전 오래됨 | Windows Update 후 재시도 |
