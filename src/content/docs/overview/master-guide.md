...
> 전체 용어정리: [용어정리](/overview/glossary/) 참고

| 용어 | 한줄 설명 |
|------|-----------|
| **Git** | 코드의 변경 이력을 기록하고 되돌리는 도구 |
| **커밋 (Commit)** | 하나의 세이브 포인트. `git commit -m "메시지"` |
| **스테이징 (Staging)** | 세이브할 파일을 골라 바구니에 담는 과정. `git add` |
| **브랜치 (Branch)** | 원본을 건드리지 않고 실험할 수 있는 갈래 |
| **리포지토리 (repo)** | Git이 관리하는 프로젝트 폴더 |
| **.gitignore** | Git이 무시할 파일 목록 |

## 1주차 실습: AI 설명 문서를 여러 언어로 만들기

> 상세 실습 가이드: [1주차: Git 되돌리기](/week1/) 참고

### 핵심 흐름

```bash
# 1) 프로젝트 만들고 Git 시작
mkdir ai-docs && cd ai-docs && git init

# 2) AI에게 한글 문서 받아서 저장 → 커밋
git add ai-intro.txt
git commit -m "한글 버전 작성"

# 3) 영어로 교체 → 커밋
git commit -am "영어 버전으로 교체"

# 4) 중국어·일본어 추가 → 각각 커밋
git commit -am "중국어 버전 추가"
git commit -am "일본어 버전 추가"

# 5) 커밋 기록 확인 → 원하는 시점으로 되돌리기
git log --oneline
git restore --source 커밋ID ai-intro.txt   # 특정 시점으로
git restore ai-intro.txt                   # 최신으로 복구
git revert HEAD                            # 커밋 후 되돌리기

# 6) 실험 브랜치로 안전하게 시도
git checkout -b experiment      # 실험 시작
git checkout main               # 안 되면 돌아오기
```
...
| `git init` | Git 저장소 시작 |
| `git status` | 상태 확인 |
| `git add [파일]` | 세이브할 파일 지정 |
| `git commit -m "[메시지]"` | 세이브 포인트 생성 |
| `git log --oneline` | 세이브 목록 보기 |
| `git restore [파일]` | 커밋 전 되돌리기 |
| `git revert HEAD` | 커밋 후 되돌리기 |
| `git checkout -b [이름]` | 실험 브랜치 만들기 |
...
1. https://www.vmware.com/products/desktop-hypervisor/workstation-and-fusion 에서 설치 파일 다운로드
2. 설치 중 "비상업적 용도" 선택 → 무료 사용
3. 재부팅

> VMware는 Broadcom 인수 이후 라이선스 정책이 변경되었습니다. 설치 시점에 무료 사용 조건을 확인하세요.
...
| 비용 | 구독($20~200/mo) 또는 API 사용량 기반 | 무료 플랜 있음 (유료 $16~20/mo) |
